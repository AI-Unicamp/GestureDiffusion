...
def main():
    ...
    dummy_motion, data_text, chunk_len, data_audio, data_audio_rep, dummy_seed, max_length  = data.dataset.gettestbatch(num_samples)
    ...
    for chunk in range(chunks_per_take): # Motion is generated in chunks, for each chunk we load the corresponding data
        ...
        for take in range(num_samples): # For each take we will load the current chunk
            inputs.append((empty, data_text[take][chunk], chunk_len, empty, data_audio_rep[take][...,chunk:chunk+chunk_len], dummy_seed))
