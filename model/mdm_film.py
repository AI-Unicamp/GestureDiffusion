...
class MDM(nn.Module):
    def __init__(self, njoints, nfeats, pose_rep, data_rep, latent_dim=256, ff_size=1024,
        ...
        super().__init__()
        print('Using MDM V3 (w/ CrossAtt+RPM+FiLM)')
        ...
        self.cl_head=8
        self.project_to_lat = nn.Linear(self.latent_dim + self.audio_feat_dim, self.latent_dim)
        self.clatt = CrossLocalFilm()

        # Positional Encodings
        self.rel_pos = SinusoidalEmbeddings(self.latent_dim // self.cl_head)

        # Self-Attention
        self.seatt = SelfAttentionFilm(num_heads, ff_size, activation, num_layers, dropout, latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        embs_seed = self.seed_pose_encoder(self.mask_cond(seed, force_mask=force_mask))     # [BS, SEED_POSES, LAT_DIM]
        ...
        xseq = self.clatt(gammas, betas, xseq, xseq, xseq,     
            packed_shape=packed_shape, mask=mask_local)         # [CHUNK_LEN, BS, LAT_DIM] 
        

        # Concat Past Information
        xseq = torch.cat((embs_seed, xseq), axis=0)              # [CHUNK_LEN+SEED_POSE, BS, LAT_DIM]   

        # Data Reshaping (Insert multiple att heads)
        xseq = xseq.permute(1, 0, 2)                                            # [BS, CHUNK_LEN+SEED_POSES, LAT_DIM]
        xseq = xseq.view(bs, nframes + self.seed_poses , self.cl_head, -1)      # [BS, CHUNK_LEN+SEED_POSES, CL_HEAD, LAT_DIM / CL_HEAD]
        xseq = xseq.permute(0, 2, 1, 3)                                         # [BS, CL_HEAD, CHUNK_LEN+SEED_POSES, LAT_DIM / CL_HEAD]
        xseq = xseq.reshape(bs*self.cl_head, nframes + self.seed_poses , -1)    # [BS * CL_HEAD, CHUNK_LEN+SEED_POSES, LAT_DIM / CL_HEAD]

        # RPE Embeddings
        pos_emb = self.rel_pos(xseq)                            # [CHUNK_LEN+SEED_POSES, BS] O CORRETO É [CHUNK_LEN+SEED_POSES, LAT_DIM / CL_HEAD]
        xseq, _ = apply_rotary_pos_emb(xseq, xseq, pos_emb)     # [LAT_DIM, CHUNK_LEN+SEED_POSES, BS]  O CORRETO É [BS * CL_HEAD, CHUNK_LEN+SEED_POSES, LAT_DIM / CL_HEAD]

        # Data Reshaping
        xseq_rpe = xseq.reshape(bs,self.cl_head,nframes+self.seed_poses,-1)     # [BS, CL_HEAD, CHUNK_LEN+SEED_POSES, LAT_DIM / CL_HEAD]
        xseq = xseq_rpe.permute(0, 2, 1, 3)                                     # [BS, CHUNK_LEN+SEED_POSES, CL_HEAD, LAT_DIM / CL_HEAD]   
        xseq = xseq.view(bs, nframes + self.seed_poses, -1)                     # [BS, CHUNK_LEN+SEED_POSES, LAT_DIM]
        xseq = xseq.permute(1, 0, 2)                                            # [CHUNK_LEN+SEED_POSES, BS, LAT_DIM]

        # Self-Attention
        output = self.seatt(gammas, betas, xseq)                    # [CHUNK_LEN+SEED_POSES, BS, LAT_DIM] 
...
class CrossLocalFilm(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self,  gammas, betas, x, y, z, packed_shape, mask_local):
        bs, _, nframes, _ = x.shape
        x = self.cross_local_attention(x, y, z, packed_shape=packed_shape, mask=mask_local) # [BS, CL_HEAD, CHUNK_LEN, LAT_DIM / CL_HEAD]
        x = x.permute(0, 2, 1, 3)                                                           # [BS, CHUNK_LEN, CL_HEAD, LAT_DIM / CL_HEAD]
        x = x.reshape(bs, nframes, -1)                                                      # [BS, CHUNK_LEN, LAT_DIM] 
        x = self.film(x, gammas, betas)                                                     # [BS, CHUNK_LEN, LAT_DIM]
        x = x.permute(1, 0, 2)                                                              # [CHUNK_LEN, BS, LAT_DIM] 
        return x
    
class SelfAttentionFilm(nn.Module):
